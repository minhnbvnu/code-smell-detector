...
let TokenizerService = class TokenizerService {
    ...
    runSingleBertTokenization(fileName, type = "1d", isPositive) {
        const cmd = `python3 src/tokenizer/tokenizer.py ${type} scanner/${isPositive ? "errors" : "functions"}/${this.fileService.escapeSpecialCharacter(fileName)}`;
        let exeRes = (0, child_process_1.execSync)(cmd, {
            encoding: "utf-8",
            maxBuffer: 1024 * 1024 * 10,
        }).toString();
