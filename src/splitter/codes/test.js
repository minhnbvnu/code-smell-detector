...
var checkControllerPresentAndSetup =
...
var TOUCH_CONTROLLER_MODEL_BASE_URL =
  AFRAME_CDN_ROOT + 'controllers/oculus/oculus-touch-controller-';
...
var OCULUS_TOUCH_CONFIG = isWebXRAvailable
  ? OCULUS_TOUCH_WEBXR
  : OCULUS_TOUCH_WEBVR;
...
var CONTROLLER_PROPERTIES = {
  ...
  'oculus-touch-v2': {
    left: {
      ...
      rayOrigin: {
        origin: { x: -0.006, y: -0.03, z: -0.04 },
        direction: { x: 0, y: -0.9, z: -1 },
      },
...
var INPUT_MAPPING = isWebXRAvailable
  ? INPUT_MAPPING_WEBXR
  : INPUT_MAPPING_WEBVR;
...
module.exports.Component = registerComponent('oculus-touch-controls', {
  ...
  loadModel: function (controller) {
    ...
    this.displayModel =
      ...
      CONTROLLER_PROPERTIES[CONTROLLER_DEFAULT];
    ...
    if (data.controllerType === 'auto') {
      var trackedControlsSystem =
        this.el.sceneEl.systems['tracked-controls-webvr'];
      // WebVR
  ...
  injectTrackedControls: function (controller) {
    ...
    var webVRId =
      data.hand === 'right' ? 'Oculus Touch (Right)' : 'Oculus Touch (Left)';
    ...
    if (this.isTouchV3orPROorPlus) {
      ...
      if (buttonMeshes) {
        if (button === 'trigger' && buttonMeshes.trigger) {
          buttonMeshes.trigger.rotation.x =
            this.originalXRotationTrigger - analogValue * (Math.PI / 26);
        }
        if (button === 'grip' && buttonMeshes.grip) {
          analogValue *= this.data.hand === 'left' ? -1 : 1;
          buttonMeshes.grip.position.x =
            this.originalXPositionGrip + analogValue * 0.004;
        }
      }
    }
  ...
  onModelLoaded: function (evt) {
    ...
    if (this.isTouchV3orPROorPlus) {
      ...
      this.originalXPositionGrip =
        buttonMeshes.grip && buttonMeshes.grip.position.x;
      buttonMeshes.trigger =
        controllerObject3D.getObjectByName('buttonTrigger');
      this.originalXRotationTrigger =
        buttonMeshes.trigger && buttonMeshes.trigger.rotation.x;
  ...
  onTouchV3orPROorPlusModelLoaded: function (evt) {
    ...
    buttonObjects.grip = controllerObject3D.getObjectByName(
      'xr_standard_squeeze_pressed_value',
    );
    buttonRanges.grip = {
      min: controllerObject3D.getObjectByName(
        'xr_standard_squeeze_pressed_min',
      ),
      max: controllerObject3D.getObjectByName(
    ...
    buttonObjects.thumbstick = controllerObject3D.getObjectByName(
      'xr_standard_thumbstick_pressed_value',
    );
    buttonRanges.thumbstick = {
      min: controllerObject3D.getObjectByName(
        'xr_standard_thumbstick_pressed_min',
      ),
      max: controllerObject3D.getObjectByName(
    ...
    buttonObjects.thumbstickXAxis = controllerObject3D.getObjectByName(
      'xr_standard_thumbstick_xaxis_pressed_value',
    );
    buttonRanges.thumbstickXAxis = {
      min: controllerObject3D.getObjectByName(
        'xr_standard_thumbstick_xaxis_pressed_min',
      ),
      max: controllerObject3D.getObjectByName(
        'xr_standard_thumbstick_xaxis_pressed_max',
      ),
    };

    buttonObjects.thumbstickYAxis = controllerObject3D.getObjectByName(
      'xr_standard_thumbstick_yaxis_pressed_value',
    );
    buttonRanges.thumbstickYAxis = {
      min: controllerObject3D.getObjectByName(
        'xr_standard_thumbstick_yaxis_pressed_min',
      ),
      max: controllerObject3D.getObjectByName(
    ...
    buttonObjects.trigger = controllerObject3D.getObjectByName(
    ...
    buttonRanges.trigger = {
      ...
      max: controllerObject3D.getObjectByName(
        'xr_standard_trigger_pressed_max',
      ),
    };
    buttonRanges.trigger.diff = {
      x:
        Math.abs(buttonRanges.trigger.max.rotation.x) -
        Math.abs(buttonRanges.trigger.min.rotation.x),
      y:
        Math.abs(buttonRanges.trigger.max.rotation.y) -
        Math.abs(buttonRanges.trigger.min.rotation.y),
      z:
        Math.abs(buttonRanges.trigger.max.rotation.z) -
        Math.abs(buttonRanges.trigger.min.rotation.z),
    };
    ...
    buttonMeshes[button1id] = controllerObject3D.getObjectByName(
      button1 + '_button',
    );
    buttonObjects[button1id] = controllerObject3D.getObjectByName(
      button1 + '_button_pressed_value',
    );
    ...
    buttonMeshes[button2id] = controllerObject3D.getObjectByName(
      button2 + '_button',
    );
    buttonObjects[button2id] = controllerObject3D.getObjectByName(
      button2 + '_button_pressed_value',
    );
  ...
  updateButtonModel: function (buttonName, state) {
    ...
    if (buttonMeshes[buttonName]) {
      color =
        state === 'up' || state === 'touchend'
          ? buttonMeshes[buttonName].originalColor || this.data.buttonColor
          : state === 'touchstart'
